...
# Serie Note
1. ___Serie Geometrica___: 
$$
\sum_k^{\infty}ax^n=a+ax+ax^2+...+ax^n
$$
 con $k>0$, è una serie geometrica se il rapporto tra due termini successivi è sempre uguale, e questo rapporto prende il nome di ___ragione $\rho$ della serie___$\frac{ax^n}{ax^{n-1}} = ax$. 
...
3. ___Serie Telescopica___: 
$$
\sum_{n=1}^\infty (a_{n+1}-a_n)=(a_2-a_1)+(a_3-a_2)+...+(a_{n+1}-a_n)
$$
oppure

$$
\sum_{n}^\infty (a_n-a_{n+1})\rightarrow \lim_{n \to +infty} \sum = \lim_{n\to +\infty}(a_1-a_{n+1})
$$
rg
...
$$
\vec x \bullet \vec y\ \  oppure<\vec x,\vec y> \ =||\vec x||\cdot ||\vec y|| \cdot cos \gamma = \ x_1 y_1 + x_2 y_2 +\ ...\ + x_n y_n
$$
...
$$
\vec{v} \times \vec{w} \ oppure \ \vec{v} \land \vec{w}= ||\vec{v}|| \cdot || \vec{w}|| \sin(θ)
$$
La direzione del vettore risultante è la direzione ___ortogonale___ al piano che contiene i vettori $\vec{v}$ e $\vec{w}$.

...
Usando il modello a caduta costante per il diodo ([[Diodo#Modello a caduta Costante]]):
Data $V_S=V_M \sin (\omega t)$ in entrata, notiamo che la $V_u$ in uscita è unidirezionale, ma è diversa da zero solo durante i semi periodi positivi della tensione in entrata, inoltre, a causa della caduta di potenziale sul Diodo ($\sim0.7 \text V$) notiamo che sono presenti un **ritardo di conduzione** e un **anticipo di spegnimento**, entrambi del valore di $\Delta t$.
...
Come abbiamo detto, durante i semiperiodi negativi è il condensatore a fornire la differenza di potenziale, che però diminuisce al diminuire della carica del dispositivo.
